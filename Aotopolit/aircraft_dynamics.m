...
function sys = mdlDerivatives(t, x, uu, P)
    ...
    neddot = [cos(theta)*cos(psi) sin(phi)*sin(theta)*cos(psi)-cos(phi)*sin(psi) cos(phi)*sin(theta)*cos(psi)+sin(theta)*sin(psi); cos(theta)*sin(psi) sin(phi)*sin(theta)*sin(psi)+cos(phi)*cos(psi) cos(phi)*sin(theta)*sin(psi)-sin(phi)*cos(psi); -sin(theta) sin(phi)*cos(theta) cos(phi)*cos(theta)]*[u; v; w];

    % Time derivative of u, v, w
    veldot = [r*v-q*w;p*w-r*u;q*u-p*v]+1/P.mass*[fx;fy;fz];

    % Time derivative of phi, theta, psi
    attdot = [1 sin(phi)*tan(theta) cos(phi)*tan(theta); 0 cos(phi) -sin(phi); 0 sin(phi)/cos(theta) cos(phi)/cos(theta)]*[p; q; r];
    ...
    attratdot = [gam1*p*q-gam2*q*r; gam5*p*r-gam6*(p*p-r*r); gam7*p*q-gam1*q*r]+[gam3*ell+gam4*n; (1/P.Jy)*m; gam4*ell+gam8*n];
